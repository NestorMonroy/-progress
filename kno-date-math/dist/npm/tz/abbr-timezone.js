 ...
 * @example
 * ```ts-no-run
 * import { abbrTimezone } from '@progress/kno-date-math';
 * import '@progress/kno-date-math/tz/Europe/Sofia';
 *
...
var abbrTimezone = function (timezone, date) {
    ...
    var _a = (0, zone_and_rule_1.zoneAndRule)(timezone, date), zone = _a.zone, rule = _a.rule;
    var base = zone[2];
    if (base.indexOf("/") >= 0) {
        ...
        var hoursPart = (0, pad_number_1.padNumber)(hours, 2);
        var signPart = hours > 0 ? '+' : '';
        var minutes = Math.abs(Math.trunc(zone[0] % 60));
        var minutesPart = minutes === 0 ? '' : (0, pad_number_1.padNumber)(minutes, 2);
        return base.replace("%z", "".concat(signPart).concat(hoursPart).concat(minutesPart));
    }
    return base;
};
exports.abbrTimezone = abbrTimezone;
