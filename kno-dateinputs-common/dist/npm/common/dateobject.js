...
var DateObject = /** @class */ (function () {
    function DateObject(_a) {
        ...
        if (!value) {
            ...
            this._value = (0, kno_date_math_1.cloneDate)(value);
        }
    }
    ...
    DateObject.prototype.setValue = function (value) {
        ...
        else if (!(0, kno_date_math_1.isEqual)(value, this._value)) {
            this._value = (0, kno_date_math_1.cloneDate)(value);
            this.modifyExisting(true);
        }
        else if ((0, kno_date_math_1.isEqual)(value, this._value) && this.dayPeriods) {
    ...
    DateObject.prototype.getValue = function () {
        ...
        return (0, kno_date_math_1.cloneDate)(this.value);
    };
    /**
     * @hidden
     */
    DateObject.prototype.createDefaultDate = function () {
        ...
        return (0, kno_date_math_1.createDate)(DEFAULT_LEAP_YEAR, 0, 31);
    };
    /**
     * @hidden
     */
    DateObject.prototype.getDefaultDate = function () {
        return (0, kno_date_math_1.getDate)(this.createDefaultDate());
    };
    /**
     * @hidden
     */
    ...
    DateObject.prototype.modifyPart = function (symbol, offset) {
        ...
        var newValue = (0, kno_date_math_1.cloneDate)(this.value);
        ...
        if (!this.autoCorrectParts && (isDay || isMonth)) {
            ...
            var dateCandidate = (0, kno_date_math_1.createDate)(year, month, day, hour, minutes, seconds, milliseconds);
            ...
            if (this.getValue() && (0, utils_2.areDatePartsEqualTo)(dateCandidate, year, month, day, hour, minutes, seconds, milliseconds)) {
                newValue = (0, kno_date_math_1.cloneDate)(dateCandidate);
                    ...
                    if (this.getExisting("d")) {
                        if (dateCandidateExists) {
                            ...
                            this.setInvalidDatePart(symbol, {
                                value: month,
                                date: (0, kno_date_math_1.cloneDate)(newValueCandidate),
                                startDateOffset: offset,
                                startDate: (0, kno_date_math_1.cloneDate)(this.value)
                            });
                            this.setExisting(symbol, false);
                        }
                    }
                        ...
                        this.resetInvalidDateSymbol(symbol);
                        newValue = (0, kno_date_math_1.cloneDate)(newValueCandidate);
                    }
                }
                else {
                    invalidDateFound = true;
                ...
                if (newValueCandidate.getDate() === day) {
                    ...
                    this.setInvalidDatePart(symbol, {
                        value: day,
                        date: (0, kno_date_math_1.cloneDate)(this.value),
                        startDateOffset: offset,
                        startDate: (0, kno_date_math_1.cloneDate)(this.value)
                    });
                    this.setExisting(symbol, false);
            ...
            switch (symbol) {
                case 'y':
                    ...
                    break;
                case 'M':
                    newValue = (0, kno_date_math_1.addMonths)(this.value, offset);
                    break;
                case 'd':
                case 'E':
    ...
    DateObject.prototype.parsePart = function (_a) {
        ...
        if (isInCaretMode || (!isInCaretMode && !this.autoCorrectParts)) {
            ...
            if (hasFixedFormat) {
                ...
                // With autoCorrectParts=false, the second loop below resets datePartText using originalFormat,
                // early return here blocks valid input (e.g. second digit in month with formatPlaceholder="wide").
            ...
            if (!hasFixedFormat || (hasFixedFormat && !this.autoCorrectParts)) {
                ...
                suffix = "";
                replaced = false;
                // baseDate is character-aligned with convertedBaseFormat, not originalFormat.
                // When placeholder text expands a segment, originalFormat[i] diverges from baseDate[i].
                var baseSymbolStart = convertedBaseFormat.indexOf(symbol);
                var symbolCharIdx = 0;
                var segmentCharIndex_1 = 0;
                // Iterate originalFormat because rawInputValue is co-indexed with originalFormat, not convertedBaseFormat.
            ...
            if (!parsedDate && !isNaN(middleNumber) && isCurrentCharParsable && this.autoCorrectParts) {
                if (symbol === MONTH_SYMBOL && !monthByChar) {
                    ...
                    if (monthNumber > -1 && monthNumber < 12) {
                        parsedDate = (0, kno_date_math_1.cloneDate)(this.value);
                        parsedDate.setMonth(monthNumber);
                        if (parsedDate.getMonth() !== monthNumber) {
                            parsedDate = (0, kno_date_math_1.lastDayOfMonth)((0, kno_date_math_1.addMonths)(parsedDate, -1));
                        }
                    }
                }
                if (symbol === 'y') {
                    parsedDate = (0, kno_date_math_1.createDate)(parseInt(middle, 10), this.month ? this.value.getMonth() : 0, this.date ? this.value.getDate() : 1, this.hours ? this.value.getHours() : 0, this.minutes ? this.value.getMinutes() : 0, this.seconds ? this.value.getSeconds() : 0, this.milliseconds ? this.value.getMilliseconds() : 0);
                    if (((isInCaretMode && (0, utils_2.isValidDate)(parsedDate)) ||
        ...
        if (!this.autoCorrectParts) {
            ...
            if ((0, utils_2.isNumber)(parsedValue) && (0, utils_2.isParseableToInt)(textToParse)) {
                if ((symbol === "d" && (parsedValue <= 0 || parsedValue > 31)) ||
                    (symbol === "M" && (parsedValue <= 0 || parsedValue > 12))) {
                    if (isInCaretMode) {
                ...
                var isMonth = symbol === "M";
                var isDay = symbol === "d";
                var newValue = (0, kno_date_math_1.cloneDate)(this._value);
                var invalidDateParts = this._partiallyInvalidDate.invalidDateParts || {};
                ...
                var lengthToCheck = resetSegmentValue ? (0, utils_1.unpadZero)(textToParse).length : textToParse.length;
                if (symbol === "M") {
                    ...
                    // don't switch if the user has only typed one digit and hasn't reached a value that requires switching (>= 4)
    ...
    DateObject.prototype.merge = function (text, mask) {
        ...
        for (var formatSymbolIndex = format.length - 1; formatSymbolIndex >= 0; formatSymbolIndex--) {
            ...
            if (this.knownParts.indexOf(format[formatSymbolIndex]) === -1 || this.getExisting(format[formatSymbolIndex])) {
                ...
                if (this.autoCorrectParts || (!this.autoCorrectParts && !(0, utils_2.isPresent)(this.getInvalidDatePartValue(symbol)))) {
    ...
    DateObject.prototype.modifyDateSymbolWithOffset = function (date, symbol, offset) {
        var newValue = (0, kno_date_math_1.cloneDate)(date);
        var timeModified = false;
        switch (symbol) {
            case 'y':
                newValue.setFullYear(newValue.getFullYear() + offset);
    ...
    DateObject.prototype.modifyDateSymbolWithValue = function (date, symbol, value) {
        ...
        switch (symbol) {
            case 'y':
                ...
                break;
            case 'M':
                newValue = (0, kno_date_math_1.addMonths)(date, value - date.getMonth());
                break;
            case 'd':
            case 'E':
