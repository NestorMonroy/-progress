import { cloneDate, firstDayOfMonth, Day } from '@progress/kno-date-math';
...
const weekInYear = (date, weekStart) => {
    ...
    const days = Math.floor((date.getTime() - Date.UTC(year, 0, 1, -6)) / 86400000);
...
/** @hidden */
export const byMonth = (date, _end, rule) => {
...
export const byMonthDay = (date, end, rule) => {
    ...
    while (date <= end) {
        ...
        const days = ruleValues(rule.byMonthDay, date.getUTCDate(), normalize);
        if (days === null) {
            return changed;
        }
        changed = true;
...
export const byYearDay = (date, end, rule) => {
    ...
    while (date < end) {
        ...
        const days = ruleValues(rule.byYearDay, dayInYear(date), normalize);
        if (days === null) {
            return changed;
        }
        changed = true;
...
/** @hidden */
export const byHour = (date, _end, rule) => {
    const hourRules = rule.byHour;
    let changed = false;
...
/** @hidden */
export const byMinute = (date, _end, rule) => {
