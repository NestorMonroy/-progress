...
export function expand(rule, options) {
    ...
    if (rule.start.getTime() > rule.end.getTime()) {
        return {
            success: false,
            errorMessage: `Invalid recurrence rule: Start date (${rule.start.toString()})` +
                `is greater than End date ${rule.start.toString()}`,
            events: []
        };
    }
    if (!freq) {
    ...
    start = ZonedDate.fromUTCDate(next, start.timezone);
    while (start <= end) {
        const endDate = ZonedDate.fromLocalDate(new Date(start.toLocalDate().getTime() + eventDuration), start.timezone);
