import { MS_PER_HOUR, MS_PER_MINUTE, ZonedDate } from '@progress/kno-date-math';
...
export function expand(rule, options) {
    ...
    start = ZonedDate.fromUTCDate(next, start.timezone);
    while (start <= end) {
        var endDate = ZonedDate.fromLocalDate(new Date(start.toLocalDate().getTime() + eventDuration), start.timezone);
