...
var normalizeDST = function (date) {
    return kno_date_math_1.ZonedDate.fromLocalDate(date.toLocalDate(), date.timezone);
};
var getTimeAfterMidnight = function (date) {
    return date.getHours() * kno_date_math_1.MS_PER_HOUR +
        date.getMinutes() * kno_date_math_1.MS_PER_MINUTE +
...
var startPeriodByFreq = function (start, rule) {
    ...
    switch (rule.freq) {
        ...
        case 'monthly':
            ...
            break;
        case 'weekly':
            (0, utils_1.setDayOfWeek)(date, rule.weekStart, -1);
            break;
        default:
            break;
    }
    ...
    return kno_date_math_1.ZonedDate.fromUTCDate(date, start.timezone);
};
...
function expand(rule, options) {
    ...
    var eventDuration = (0, utils_1.duration)(rule.start, rule.end);
    var endPeriod;
    if (rule.start.getTime() > rule.end.getTime()) {
        return {
            success: false,
            errorMessage: "Invalid recurrence rule: Start date (".concat(rule.start.toString(), ")") +
                "is greater than End date ".concat(rule.start.toString()),
            events: []
        };
    }
    if (!freq) {
        return {
            success: false,
            errorMessage: "Invalid recurrence frequency \"".concat(freqName, "\""),
            events: []
        };
    }
    var events = [];
    ...
    if (start.getTime() < eventStart.getTime() || count || rule.interval > 1 || useEventStart) {
        ...
        start = kno_date_math_1.ZonedDate.fromUTCDate(startAdj, start.timezone);
    }
    ...
    if (positions) {
        ...
        if (diff < 0) {
            ...
            end = kno_date_math_1.ZonedDate.fromUTCDate(endAdj, end.timezone);
        }
        rule._startPeriod = start.clone();
    ...
    if (freq.setup) {
        ...
        start = kno_date_math_1.ZonedDate.fromUTCDate(next, start.timezone);
    }
    next = start.toUTCDate();
    freq.limit(next, end.toUTCDate(), rule);
    start = kno_date_math_1.ZonedDate.fromUTCDate(next, start.timezone);
    while (start <= end) {
        var endDate = kno_date_math_1.ZonedDate.fromLocalDate(new Date(start.toLocalDate().getTime() + eventDuration), start.timezone);
