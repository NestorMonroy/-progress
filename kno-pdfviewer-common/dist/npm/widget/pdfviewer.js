...
const kendo_file_saver_1 = require("@progress/kno-file-saver");
...
class PdfViewer extends main_1.Component {
    ...
    printFile() {
        ...
        if (this.options.zoomBeforePrint) {
            // such option existed in the Kno jQuery version internally
    ...
    scrollPageIntoView({ pageNumber, destArray = null, allowNegativeOffset = false, ignoreDestinationZoom = false }) {
        ...
        if (pageView.isBlank()) {
            this.drawPageAsync({ pageIndex: pageNumber - 1 }).then(() => {
                this.scrollPageIntoView({ pageNumber, destArray, allowNegativeOffset, ignoreDestinationZoom });
            });
            return;
        }
