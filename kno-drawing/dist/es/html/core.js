...
import { parseColor as utils_parseColor, support, template as compileTemplate, resolveElementColor } from "../common";
...
var kno_PSEUDO_ELEMENT = "kno-PSEUDO-ELEMENT";
const kno_BULLET_TYPE = 'data-kno-bullet-type';
...
function closest(el, selector) {
    ...
    // https://github.com/tlk/kno/issues/6721
...
var cloneNodes = (function($){
    if ($) {
        // if we have kno and jQuery, use this version as it will
        // maintain proper links between cloned element and kno
        // widgets (i.e. it clones jQuery data(), which isn't the same
        // as element's data attributes).
        // https://github.com/tlk/kno-ui-core/issues/2750
        return function cloneNodes(el) {
            ...
            if (el.nodeType == 1 /* Element */) {
                ...
                if (/^canvas$/i.test(el.tagName)) {
                    ...
                    // into the DOM.  https://github.com/tlk/kno/issues/5409
                    ...
                    // https://github.com/tlk/kno-ui-core/issues/6121
                    clone.removeAttribute("loading");
                }

                if (el._knoExportVisual) {
                    clone._knoExportVisual = el._knoExportVisual;
                }
        ...
        return function cloneNodes(el) {
            const clone = (function dive(node){
                ...
                if (node._knoExportVisual) {
                    clone._knoExportVisual = node._knoExportVisual;
                }
            ...
            // re-draw canvases - https://github.com/tlk/kno/issues/4872
            ...
            // https://github.com/tlk/kno/issues/5409
...
})(typeof window !== "undefined" && window.kno && window.kno.jQuery);
...
function drawDOM(element, options) {
    ...
    function handlePageBreaks(callback, element, forceBreak, pageWidth, pageHeight, margin, options) {
        ...
        var container = doc.createElement("kno-PDF-DOCUMENT");
        var adjust = 0;

        // make sure <tfoot> elements are at the end (Grid widget
        // places TFOOT before TBODY, tricking our algorithm to
        // insert a page break right after the header).
        // https://github.com/tlk/kno/issues/4699
        ...
        slice(copy.querySelectorAll("ol")).forEach(function(ol){
            slice(ol.children).forEach(function(li, index){
                li.setAttribute("kno-split-index", index);
            });
        });
        ...
        function keepTogether(el) {
            ...
            return (el.getAttribute("data-kno-chart") ||
        ...
        function breakAtElement(el) {
            ...
            if (options.repeatHeaders) {
                ...
                // If we break page in a kno Grid, repeat its header.  This ugly hack is
            ...
            for (var i = 0; i < breakRowIndex; i++) {
                ...
                for (var j = 0; j < cells.length; j++) {
                    ...
                    if (i + rowspan > breakRowIndex) {
                        ...
                        const cellWidth = getPropertyValue(getComputedStyle(cell), "width");
        ...
        function makePage() {
            var page = doc.createElement("kno-PDF-PAGE");
            setCSS(page, {
                display  : "block",
                boxSizing: "content-box",
        ...
        function preventBulletOnListItem(el) {
            ...
            // https://github.com/tlk/kno-ui-core/issues/2732
            var li = closest(el, "li");
            if (li) {
                li.setAttribute("kno-no-bullet", "1");
...
function cacheImages(elements, callback) {
    ...
    function next() {
        if (--count <= 0) {
            ...
            // not working in all cases /issues/5399), so this
...
function _renderWithPseudoElements(element, group) {
    if (element.tagName == kno_PSEUDO_ELEMENT) {
        _renderElement(element, group);
        return;
    }
    let fake = [];
    function pseudo(kind, place) {
        ...
        if (content && content != "normal" && content != "none" && style.width != "0px") {
            let psel = element.ownerDocument.createElement(kno_PSEUDO_ELEMENT);
    ...
    if (fake.length > 0) {
        ...
        element.className += " kno-pdf-hide-pseudo-elements";
        _renderElement(element, group);
...
function _renderElement(element, group) {
    ...
    var backgroundColor = parseColor(backgroundColorValue);
    ...
    //
    // https://github.com/tlk/kno/issues/5232
    // https://github.com/tlk/kno-ui-core/issues/1868
    //
    ...
    if (display == "table-row") {
        ...
        // previously rendered cell.  https://github.com/tlk/kno/issues/4881
        boxes = [];
    ...
    // images inside.  https://github.com/tlk/kno-ui-core/issues/3359
    ...
    if (boxes.length > 0 && display == "list-item" && !element.getAttribute("kno-no-bullet")) {
        drawBullet(boxes[0]);
    }
    ...
    function drawOneBackground(group, box, background, backgroundRepeat, backgroundPosition, backgroundOrigin, backgroundSize) {
        ...
        function drawBackgroundImage(group, box, img_width, img_height, renderBG) {
            ...
            // https://github.com/tlk/kno-ui-core/issues/2786
            //
    ...
    function drawBullet() {
        ...
        function _drawBullet(f) {
            saveStyle(element, function(){
                ...
                var bullet = element.ownerDocument.createElement(kno_PSEUDO_ELEMENT);
        ...
        function elementIndex(f) {
            ...
            var k = element.getAttribute("kno-split-index");
            if (k != null) {
        ...
        switch (listStyleType) {
          ...
          case "square":
            _drawBullet(function(bullet){
                bullet.innerHTML = '&nbsp;';
                bullet.setAttribute(kno_BULLET_TYPE, listStyleType);
            });
            break;
...
function maybeRenderWidget(element, group) {
    let visual = null;
    if (window.kno && window.kno.jQuery && element.getAttribute(window.kno.attr("role"))) {
        let widget = window.kno.widgetInstance(window.kno.jQuery(element));
    ...
    if (visual === null && element._knoExportVisual) {
        const rect = element.getBoundingClientRect();
        const size = {
            width: rect.width,
            height: rect.height
        };
        visual = element._knoExportVisual(size);
    }
...
function maybeRenderBullet(element, group) {
    const bulletType = element.getAttribute(kno_BULLET_TYPE);
...
function renderFormField(element, group) {
    ...
    var doc = element.ownerDocument;
    var el = doc.createElement(kno_PSEUDO_ELEMENT);
    var option;
    ...
    if (tag == "select") {
        if (element.multiple) {
            for (var i = 0; i < element.options.length; ++i) {
                option = doc.createElement(kno_PSEUDO_ELEMENT);
...
function renderText(element, node, group) {
    ...
    // https://github.com/tlk/kno/issues/5740
    ...
    function actuallyGetRangeBoundingRect(range) {
        ...
        if (microsoft || browser.chrome || browser.safari) {
            ...
            // one.  https://github.com/tlk/kno/issues/4674

            // Actually Chrome 50 got worse, since the rectangles can now have the width of a
            // full character, making it hard to tell whether it's a bogus rectangle or valid
            // selection location.  The workaround is to ignore rectangles that fall on the
            // previous line.  https://github.com/tlk/kno/issues/5740
