[![npm version](https://badge.fury.io/js/%40progress%2Fkno-intl.svg)](https://badge.fury.io/js/%40telerik%2Fkno-intl)


# Kno UI Internationalization

This repository contains the source code and documentation of the Kno UI Internationalization package.
...
To apply the methods for different locales, load the `likelySubtags` and the locale data by using the [`load`]-intl/blob/master/docs/cldr/api.md#load) method.
...
```javascript
import { load } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [CLDR Data]-intl/blob/master/docs/cldr/index.md).
...
```js
import { parseDate } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [date parsing]-intl/blob/master/docs/date-parsing/index.md).
...
```js
import { formatDate } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [date formatting]-intl/blob/master/docs/date-formatting/index.md).
...
```js
import { parseNumber } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [number parsing]-intl/blob/master/docs/num-parsing/index.md).
...
```js
import { formatNumber } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [number formatting]-intl/blob/master/docs/num-formatting/index.md).
...
```js
import { format, toString } from '@progress/kno-intl';
...
For more examples and available configuration options, refer to the article on [general formatting]-intl/blob/master/docs/general-formatting/index.md).

## Installation

1. The Internationalization library is published as a [scoped NPM package](https://docs.npmjs.com/misc/scope) in the [NPMJS Telerik account](https://www.npmjs.com/~tlk).

2. Download and install the module:

    ```bash
    npm install --save '@progress/kno-intl';
    ```
...
3. Once installed, import the Internationalization in your application root module:
    ...
    import { formatDate, parseDate } from '@progress/kno-intl';
    //or
    import { formatNumber, parseNumber } from '@progress/kno-intl';
    ```
    ```javascript
    // CommonJS format
    var numbers = require('@progress/kno-intl/number').numbers;
    //or
    var dates = require('@progress/kno-intl/dates').dates;
    ```
