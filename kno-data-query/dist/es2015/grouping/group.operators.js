 ...
 * ```ts
 *
 * import { groupBy } from '@progress/kno-data-query';
 *
 * const data = [
...
export const groupBy = (data, descriptors = [], transformers = identity, originalData = data) => {
    ...
    // When a custom compare is provided, extract the group value before forwarding
    // to the user's function — GroupResult objects must not leak into the callback.
    if (isPresent(descriptor.dir) || isPresent(descriptor.compare)) {
        const userCompare = descriptor.compare;
        const comparer = composeSortDescriptors([{
                field: 'value',
                dir: descriptor.dir,
                compare: userCompare ? (a, b) => userCompare(a.value, b.value) : undefined
            }]);
