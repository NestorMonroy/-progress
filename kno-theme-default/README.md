...
> * Styles for newly released Kno UI components
...
>
> All forward-looking design work is happening in **@progress/kendo-theme-meridian**, the new recommended starting point for Kno and Telerik products. Existing applications can continue on kendo-theme-default and migrate to Meridian on their own schedule.

The Kno UI Default Theme is a SCSS-based theme for the Kno UI components.
...
Kno UI Default Theme is available as an npm package.
...
* [Using the Default Theme in Angular Projects](components/styling/theme-default/)
* [Using the Default Theme in React Projects](components/styling/theme-default/)
* [Using the Default Theme in jQuery Projects](http://docs.tlk.com/kendo-ui/styles-and-layout/sass-themes)

## Customizing

You can customize Kno UI Default Theme both in terms of what gets compiled and how the theme will look.
...
Kno UI Default Theme provides many variables for customization. There are variables that control globally used theme colors like `$primary`, `$secondary`, `$success`, `$error` etc.; variables for all components such as `$component-text`, `$component-bg`; as well as variables for individual components such as `$grid-bg`, `$tabstrip-font-size` and so on.
...
Have a bug or a feature request? Please first search for existing and closed issues. If your problem or idea is not addressed yet, please open a new [bug report]-themes/issues/new?labels=bug,T:Default&template=bug_report.md) or [feature request]-themes/issues/new?labels=Enhancement,T:Default&template=feature_request.md).
